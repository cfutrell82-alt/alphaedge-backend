...
const BACKEND_URL = 'https://alphaedge-backend-uu13.onrender.com';

// ALWAYS use webhook mode — no polling, no conflicts
const bot = new TelegramBot(TOKEN, { polling: false });

// Set webhook to point to our backend
bot.setWebHook(`${BACKEND_URL}/telegram-webhook/${TOKEN}`)
  .then(() => console.log('[BOT] Webhook set successfully'))
  .catch(err => console.error('[BOT] Webhook error:', err.message));
...
async function handleMessage(msg) {
  ...
  if (text === '/start' || text.startsWith('/start')) {
    ...
    let reply = `👋 *Welcome to AlphaEdge, ${name}\\!*\n\n`;
    if (user) {
      reply += `✅ Connected — Plan: *${esc(user.plan?.toUpperCase() || 'FREE')}*\n\n`;
    } else {
      reply += `🔗 Connect your account with /connect\n\n`;
    }
    reply += `Use the menu below to get started\\.`;
    bot.sendMessage(chatId, reply, { parse_mode: 'MarkdownV2', ...mainMenu });

  } else if (text === '/signals' || text === '📡 Signals') {
    const user = await getUser(msg.from?.id);
    ...
    bot.sendMessage(chatId, '📡 *Choose a market:*', { parse_mode: 'MarkdownV2', ...signalsMenu });

  } else if (text === '/markets' || text === '📊 Markets') {
    const reply = `📊 *Market Overview*\n\n` +
      `📈 S&P 500: 5,421 \\+0\\.84%\n` +
      `₿ BTC: $67,420 \\+3\\.6%\n` +
      `Ξ ETH: $3,812 \\+2\\.1%\n` +
      `💱 EUR\\/USD: 1\\.0842 \\-0\\.14%\n` +
...
console.log('[BOT] AlphaEdgeProBot initialized in webhook mode');

module.exports = { bot, handleMessage, handleCallback };
